...
async function encryptKai(text) {
  try {
    const res = await request(
      `${API}/enc-kai?text=${encodeURIComponent(text)}`,
    );
    ...
    if (content.includes("#EXT-X-STREAM-INF")) {
      ...
      for (const line of lines) {
        if (line.includes("RESOLUTION=")) {
          ...
          const variantUrl = line.startsWith("http")
            ? line.trim()
            : baseUrl + line.trim();
          variants.push({
            url: variantUrl,
...
async function getStreams(tmdbId, type, season, episode) {
  ...
  try {
    ...
    let alData = await searchAniList(
    ...
    const serversRes = await request(
      `${KAI_AJAX}/links/list?token=${token}&_=${encToken}`,
    );
    ...
    $(".server").each((_, el) => {
      ...
      const serverType =
        $(el).closest(".server-items").attr("data-id") || "sub";
          ...
          (async () => {
            try {
              ...
              const viewRes = await request(
                `${KAI_AJAX}/links/view?id=${lid}&_=${encLid}`,
              );
