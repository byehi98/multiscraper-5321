    ...
    .then(function (response) {
      ...
      return response.text().then(function (content) {
        ...
        if (content.includes("#EXT-X-STREAM-INF:")) {
          console.log(
            `📋 Master playlist detected - parsing quality streams...`,
          );
          ...
          for (const stream of streams) {
            ...
            const cleanServerName = sourceName
              .replace(/^XPRIME\s+/i, "")
              .replace(/\s+-\s+.*$/, "");
            ...
            console.log(
              `  📊 ${quality} (${stream.resolution || "Unknown resolution"}) - ${Math.round(((stream.bandwidth || 0) / 1000000) * 10) / 10} Mbps`,
            );
          }
          ...
          resolvedStreams.sort((a, b) => {
            ...
            return (
              (qualityOrder[b.quality] || -3) - (qualityOrder[a.quality] || -3)
            );
          });
...
function resolveMultipleM3U8(links) {
  ...
  return Promise.allSettled(resolvePromises).then(function (results) {
    ...
    console.log(`\n📊 Resolution Summary:`);
    console.log(
      `✅ Successfully resolved: ${allResolvedStreams.length} streams`,
    );
    ...
    if (failedResolutions.length > 0) {
      ...
      failedResolutions.forEach((failure, index) => {
        console.log(
          `  ${index + 1}. ${failure.link.name || "Unknown"}: ${failure.error}`,
        );
      });
    }
...
function getTurnstileToken() {
  ...
  return fetch("https://enc-dec.app/api/enc-xprime", {
    ...
    .then(function (response) {
      if (!response.ok) {
        throw new Error(
          `Turnstile token API error: ${response.status} ${response.statusText}`,
        );
      }
...
function decryptXprimeResponse(encryptedData) {
  console.log(
    `[Xprime] Decrypting encrypted response (${encryptedData.length} bytes)...`,
  );
  ...
  return fetch("https://enc-dec.app/api/dec-xprime", {
    ...
    .then(function (response) {
      if (!response.ok) {
        throw new Error(
          `Decryption API error: ${response.status} ${response.statusText}`,
        );
      }
      return response.json();
    })
    .then(function (decryptedResponse) {
      if (decryptedResponse.status === 200 && decryptedResponse.result) {
        ...
        throw new Error(
          `Decryption failed: ${decryptedResponse.error || "Unknown error"}`,
        );
      }
    })
    .catch(function (error) {
...
function getXprimeServers(api) {
  ...
  console.log(
    `[Xprime] Using ${hardcodedServers.length} hardcoded servers: ${hardcodedServers.map((s) => s.name).join(", ")}`,
  );
...
function getStreams(
  ...
  console.log(
    `[Xprime] Fetching streams for TMDB ID: ${tmdbId}, Type: ${mediaType}${mediaType === "tv" ? `, S:${season}E:${episode}` : ""}`,
  );
  ...
  return getTMDBDetails(tmdbId, mediaType)
    .then((mediaInfo) => {
      ...
      console.log(
        `[Xprime] TMDB Info: "${mediaInfo.title}" (${mediaInfo.year || "N/A"})`,
      );
      console.log(
        `[Xprime] Searching for: ${mediaInfo.title} (${mediaInfo.year})`,
      );
        ...
        .then(function (api) {
          return getTurnstileToken().then(function (turnstileToken) {
            return getXprimeServers(api).then(function (servers) {
              ...
              console.log(
                `[Xprime] Processing ${servers.length} servers in parallel with turnstile token`,
              );
                ...
                return makeRequest(serverUrl, {
                  ...
                  .then(function (response) {
                    return response.text().then(function (responseText) {
                      ...
                      let data;
                      if (
                        responseText.startsWith("AQAA") ||
                        responseText.startsWith("UklGR") ||
                        (responseText.length > 100 &&
                          !responseText.includes("{"))
                      ) {
                        console.log(
                          `[Xprime] Server ${server.name}: Detected encrypted response, decrypting...`,
                        );
                        return decryptXprimeResponse(responseText).then(
                          function (decryptedData) {
                            ...
                            if (server.name === "primebox") {
                              result = processPrimeBoxResponse(
                                data,
                                serverLabel,
                                server.name,
                              );
                            } else {
                              result = processOtherServerResponse(
                                data,
                                serverLabel,
                                server.name,
                              );
                            ...
                            console.log(
                              `[Xprime] Server ${server.name}: Found ${result.links.length} links, ${result.subtitles.length} subtitles`,
                            );
                            return result;
                          },
                        );
                        ...
                        try {
                          ...
                          console.error(
                            `[Xprime] Server ${server.name}: Invalid JSON response`,
                          );
                          return { links: [], subtitles: [] };
                        }
                  ...
                  .catch(function (error) {
                    console.error(
                      `[Xprime] Error on server ${server.name}: ${error.message}`,
                    );
                    return { links: [], subtitles: [] };
                  });
              });
              ...
              return Promise.allSettled(serverPromises).then(
                function (results) {
                  // Process results
                  for (const result of results) {
                    if (result.status === "fulfilled") {
                  ...
                  console.log(
                    `[Xprime] Total found: ${allLinks.length} links, ${allSubtitles.length} subtitles`,
                  );

                  // Separate M3U8 links from direct video links
                  const m3u8Links = allLinks.filter(
                    (link) => link.type === "M3U8",
                  );
                  ...
                  if (m3u8Links.length > 0) {
                    console.log(
                      `[Xprime] Resolving ${m3u8Links.length} M3U8 playlists...`,
                    );

                    return resolveMultipleM3U8(m3u8Links)
                      .then(function (resolutionResult) {
                        if (
                          resolutionResult.success &&
                          resolutionResult.streams.length > 0
                        ) {
                          console.log(
                            `[Xprime] Successfully resolved ${resolutionResult.streams.length} quality streams`,
                          );
                          resolvedStreams = resolutionResult.streams;
                        } else {
                          console.log(
                            `[Xprime] M3U8 resolution failed, using master playlist URLs`,
                          );
                        ...
                        const formattedLinks = groupStreamsByQuality(
                          finalLinks,
                          allSubtitles,
                          mediaInfoForGrouping,
                        );
                      ...
                      .catch(function (error) {
                        console.error(
                          `[Xprime] M3U8 resolution error: ${error.message}`,
                        );
                        resolvedStreams = m3u8Links;
